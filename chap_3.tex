...
\\Il metodo \textit{buildEGraph} costruisce un \textit{DiGraph} di \textit{NetworkX} a partire dalle informazioni collezionate nel file \textit{input\_name} attraverso i seguenti passi:
\begin{enumerate}
...
\\\\$\frac{dictio\_edges[(source,dest)]}{dictio\_nodes[source]}$.
\end{itemize}
\item Utilizzando le informazioni ottenute al passo precedente, il metodo \textit{buildEGraph} popola il grafo con dei nodi, non più identificati da uno \textit{username} ma da un intero univoco e che hanno come attributo il proprio \textit{retweetcount} (i.e. il numero di volte che l'utente corrispondente al nodo ha effettuato un \textit{retweet} sul \textit{topic} in questione), e degli archi diretti, che hanno come unico attributo una probabilità di \textit{retweet}.
\end{enumerate}
