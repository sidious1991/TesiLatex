...
In questo paragrafo verrà descritta l'implementazione di tutte quelle fasi che sono successive alla raccolta dati. Fissato un certo \textit{hashtag} ed un intervallo di osservazione della discussione che ha luogo su \textit{Twitter}, i dati raccolti (come illustrato nel paragrafo precedente) devono essere tradotti in un vero e proprio \textit{retweet graph}, per il quale è possibile:
\begin{enumerate}
...
Per eseguire agevolmente tutte le operazioni di manipolazione e gestione dei grafi utili al raggiungimento degli obiettivi sopra menzionati, è stato necessario trattare i \textit{retweet graphs} con un formato direttamente comprensibile dal linguaggio \textit{Python}. La fase di raccolta dati, illustrata nel paragrafo precedente, al termine della sua esecuzione si preoccupa di rendere persistenti le informazioni raccolte: tuttavia tali informazioni non sono manipolabili agevolmente via codice e pertanto necessitano di essere tradotte in una vera e propria struttura dati, che rappresenta il \textit{retweet graph} a cui si riferiscono.
...
\begin{enumerate}
\item \textit{parsare} il contenuto del file di testo in input, creato nella fase di raccolta dati, per ottenere la struttura dati che rappresenta il \textit{retweet graph} corrispondente;
\item \textit{serializzare} la struttura dati creata, in modo da averne disponibilità anche al termine dell'esecuzione.
\end{enumerate}
...
Il \textit{core} della classe \textit{EndorsementGraph} è il metodo \textit{buildEGraph}, il quale si occupa di effettuare questa \textit{traduzione} e di serializzare la struttura dati ottenuta, per poi memorizzare il risultato della serializzazione nella \textit{directory} il cui \textit{path} è specificato come parametro.
